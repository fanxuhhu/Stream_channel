...
SUBROUTINE Cross_Sectional_Tau(Surface,Discharge)
    ...
    INTEGER  , ALLOCATABLE , DIMENSION(:)             :: WetPoints
...
! ALLOCATE arrays
    ...
    ALLOCATE(WetPoints(ML:MU),stat=status,errmsg=err_msg)
...
! get the water depth of the cross-section
    ...
    DO i = ML , MU-1
        DO j = NL , NU
            !TTDepthQ(i,j) = 0.5D0*(DEP0(i,j) + Surface(i) + DEP0(i+1,j) + Surface(i+1))
            IF(Discharge(i)>=0.D0) THEN
                TTDepthQ(i,j) = DEP0(i,j) + Surface(i)
            !ELSE IF(Discharge(i)<0.D0) THEN
            !    TTDepthQ(i,j) = DEP0(i+1,j) + Surface(i+1)
            ELSE
                TTDepthQ(i,j) = 0.5D0*(DEP0(i,j) + Surface(i) + DEP0(i+1,j) + Surface(i+1))
            !TTDepthQ(i,j) =DEP0(i,j) + Surface(i)
            END IF
...
! DEALLOCATE arrays
    ...
    DEALLOCATE(WetPoints,stat=status,errmsg=err_msg)
